<?php
    ...
    if(isset($_REQUEST['s'])){
        ...
        $query = "SELECT id,shortDis,rating,review,manufacturer,thumbnail,price FROM product_details WHERE category LIKE '%" . $search . "%' OR primaryCategory LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR shortDis LIKE '%" . $search . "%' LIMIT 0,2";
        $result = mysqli_query($conn, $query);
        $products = mysqli_fetch_all($result, MYSQL_ASSOC);
        
        $query = "SELECT DISTINCT manufacturer FROM product_details WHERE category LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR primaryCategory LIKE '%" . $search . "%' OR shortDis LIKE '%" . $search . "%'";
        $result = mysqli_query($conn, $query);
        $manufacturers = mysqli_fetch_all($result, MYSQL_ASSOC);
    
        $query = "SELECT DISTINCT price FROM product_details WHERE category LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR primaryCategory LIKE '%" . $search . "%' OR shortDis LIKE '%" . $search . "%'";
        $result = mysqli_query($conn, $query);
...
<body>
    ...
    <?php else: 
        $categories = ['Electronics', 'Stationaries', 'Clothes and Garments', 'Utensiles'];
    ?>
        <?php foreach($categories as $category): 
             $query = "SELECT id,shortDis,rating,review,manufacturer,thumbnail,price FROM product_details WHERE primaryCategory LIKE '%" . $category . "%' OR category LIKE '%" . $category . "%'  LIMIT 4";
             $result = mysqli_query($conn, $query);
             $products = mysqli_fetch_all($result, MYSQL_ASSOC);
        ?>
